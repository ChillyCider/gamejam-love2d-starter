...
GAME_WIDTH=$(cat conf.lua | gawk 'match($0, /\.width = ([0-9]+)/, m) { print m[1] }')
GAME_HEIGHT=$(cat conf.lua | gawk 'match($0, /\.height = ([0-9]+)/, m) { print m[1] }')
GAME_TITLE=$(cat conf.lua | gawk 'match($0, /\.title = "([^"]+)"/, m) { print m[1] }')
